...
if "django.middleware.security.SecurityMiddleware" not in MIDDLEWARE:  # type: ignore[used-before-def]
    MIDDLEWARE.insert(0, "django.middleware.security.SecurityMiddleware")  # type: ignore[used-before-def]
...
# 确保 whitenoise 在中间件中
if "whitenoise.middleware.WhiteNoiseMiddleware" not in MIDDLEWARE:  # type: ignore[used-before-def]
    # 找到 SecurityMiddleware 的位置，在其后插入
    try:
        security_index = MIDDLEWARE.index("django.middleware.security.SecurityMiddleware")  # type: ignore[used-before-def]
        MIDDLEWARE.insert(security_index + 1, "whitenoise.middleware.WhiteNoiseMiddleware")  # type: ignore[used-before-def]
    except ValueError:
        MIDDLEWARE.insert(0, "whitenoise.middleware.WhiteNoiseMiddleware")  # type: ignore[used-before-def]
...
if not _is_build_key:
    if (
        not SECRET_KEY or SECRET_KEY == "your-secret-key-here" or SECRET_KEY.startswith("django-insecure-")
    ):  # nosec B105,B106
        raise ImproperlyConfigured(
            "SECRET_KEY 未设置或使用了不安全的默认值（含 django-insecure- 前缀），" "请在 .env 文件中设置安全的密钥！"
        )
