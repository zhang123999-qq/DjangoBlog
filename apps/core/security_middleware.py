...
def custom_404_view(request, exception=None):
    ...
    # 检查是否是路径遍历尝试
    is_traversal_attempt = any(
        pattern in path for pattern in ["/etc/", "/windows/", "/proc/", "../", "..\\", ".env", "settings.py"]
    )

    if is_traversal_attempt:
        logger.warning(f"[安全] 路径遍历尝试被阻止: {request.path}")
        # 对可疑请求返回简单的404，不暴露任何信息
        return HttpResponseNotFound("<h1>404 Not Found</h1>", content_type="text/html", status=404)
