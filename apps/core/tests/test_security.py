...
class TestCSRFProtection:
    ...
    def test_post_requires_csrf(self, client):
        ...
        response = client.post("/accounts/login/", {"username": "test", "password": "test"}, enforce_csrf_checks=True)
        # 应该返回 403 或重定向
...
class TestXSSProtection:
    ...
    def test_xss_in_comment_escaped(self, client, user):
        ...
        post = Post.objects.create(title="测试文章", content="内容", author=user, category=category, status="published")

        # 创建包含 XSS 的评论
        xss_payload = '<script>alert("XSS")</script>'
        client.login(username="testuser", password="TestPassword123!")

        response = client.post(f"/blog/post/{post.slug}/", {"content": xss_payload}, follow=True)
...
class TestSQLInjection:
    ...
    def test_sql_injection_in_login(self, client):
        """测试登录 SQL 注入"""
        # 尝试 SQL 注入
        client.post(reverse("accounts:login"), {"username": "admin' OR '1'='1", "password": "anything"})
        # 不应该登录成功
...
class TestFileUpload:
    ...
    def test_dangerous_file_blocked(self, client, user):
        ...
        php_file = SimpleUploadedFile("test.php", b"<?php echo 'hello'; ?>", content_type="application/x-php")
...
class TestAxesProtection:
    ...
    def test_login_failure_tracking(self, client):
        ...
        for _ in range(6):  # 超过阈值
            client.post(reverse("accounts:login"), {"username": "nonexistent", "password": "wrongpassword"})

        # 下一次尝试应该被锁定
        response = client.post(reverse("accounts:login"), {"username": "nonexistent", "password": "wrongpassword"})
        # 可能显示锁定消息或重定向
