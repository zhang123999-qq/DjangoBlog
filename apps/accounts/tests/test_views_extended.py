...
class TestLockoutMechanism:
    ...
    def test_login_lockout_after_failed_attempts(self):
        ...
        # 创建用户
        User.objects.create_user(username="locktest", password="correctpassword", email="locktest@example.com")

        # 尝试错误登录多次
        for i in range(3):
            response = self.client.post(
                self.login_url,
                {"username": "locktest", "password": "wrongpassword", "captcha": "123456"},  # 测试环境可能跳过验证码
            )
            # 应该返回登录页面（带错误）
            assert response.status_code == 200
        ...
        # 第4次尝试应该被锁定
        response = self.client.post(
            self.login_url, {"username": "locktest", "password": "wrongpassword", "captcha": "123456"}
        )
        # 应该返回登录页面（带锁定提示）
        assert response.status_code == 200
        # 注意：实际锁定逻辑可能依赖于验证码系统
...
class TestUserProfile:
    ...
    def test_profile_view_shows_pending_content(self):
        ...
        Comment.objects.create(post=post, user=self.user, content="待审核评论", review_status="pending")
...
class TestUserLogin:
    ...
    def test_login_view_post_success(self):
        ...
        response = self.client.post(
            self.login_url,
            {"username": "testuser", "password": "testpass123", "captcha": "123456"},  # 测试环境可能跳过验证码
        )
        # 登录成功应该重定向
        assert response.status_code == 302
    ...
    def test_login_view_post_invalid(self):
        """测试登录失败（无效凭据）"""
        response = self.client.post(
            self.login_url, {"username": "testuser", "password": "wrongpassword", "captcha": "123456"}
        )
        # 应该返回登录页面（带错误）
        assert response.status_code == 200
        assert "form" in response.context
