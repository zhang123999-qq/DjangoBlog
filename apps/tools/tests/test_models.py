...
class TestToolConfigModel:
    ...
    def test_create_toolconfig(self):
        """测试创建工具配置"""
        config = ToolConfig.objects.create(name="测试工具", slug="test-tool", is_enabled=True, sort_order=1)
    ...
    def test_toolconfig_cache_clear(self):
        ...
        # 创建工具配置（应该清除缓存）
        ToolConfig.objects.create(name="测试工具", slug="test-tool")
    ...
    def test_toolconfig_update_cache_clear(self):
        """测试更新时清除缓存"""
        # 创建工具配置
        config = ToolConfig.objects.create(name="测试工具", slug="test-tool")
...
class TestToolConfigProperties:
    ...
    def test_toolconfig_enabled_status(self):
        """测试启用状态"""
        config = ToolConfig.objects.create(name="启用工具", is_enabled=True)
        assert config.is_enabled is True
    ...
    def test_toolconfig_disabled_status(self):
        """测试禁用状态"""
        config = ToolConfig.objects.create(name="禁用工具", is_enabled=False)
        assert config.is_enabled is False
