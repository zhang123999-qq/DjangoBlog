...
class LoremForm(forms.Form):
    ...
    text_type = forms.ChoiceField(choices=TYPE_CHOICES, initial="paragraphs", label="Type")
    count = forms.IntegerField(min_value=1, max_value=50, initial=3, label="Count")
    start_with_lorem = forms.BooleanField(required=False, initial=True, label='Start with "Lorem ipsum"')
...
def generate_paragraph(min_sentences=4, max_sentences=7, start_with_lorem=True, language="la", first_paragraph=False):
    ...
    for i in range(sentence_count):
        ...
        sentences.append(
            generate_sentence(start_with_lorem=start_with_lorem, language=language, first_sentence=is_first)
        )
...
def process(form):
    ...
    if text_type == "paragraphs":
        for i in range(count):
            para = generate_paragraph(start_with_lorem=start_with_lorem, language=language, first_paragraph=(i == 0))
            result.append(para)
        text = "\n\n".join(result)

    elif text_type == "sentences":
        for i in range(count):
            sent = generate_sentence(start_with_lorem=start_with_lorem, language=language, first_sentence=(i == 0))
            result.append(sent)
        text = " ".join(result)
