...
class JsonFormatterEnhancedForm(forms.Form):
    ...
    sort_keys = forms.BooleanField(
        label="按键排序", initial=False, required=False, widget=forms.CheckboxInput(attrs={"class": "form-check-input"})
    )
...
class JsonFormatterEnhancedTool(BaseTool):
    ...
    def handle(self, request, form):
        ...
        try:
            ...
            result = json.dumps(data, indent=indent_str, ensure_ascii=False, sort_keys=sort_keys)
