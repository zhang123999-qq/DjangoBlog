...
class MarkdownEditorForm(forms.Form):
    ...
    markdown_text = forms.CharField(
        ...
        widget=forms.Textarea(
            attrs={"rows": 15, "class": "form-control font-monospace", "placeholder": "在此输入Markdown内容..."}
        ),
        required=True,
    )
...
class MarkdownEditorTool(BaseTool):
    ...
    def handle(self, request, form):
        ...
        try:
            # 转换为HTML
            html_content = markdown.markdown(markdown_text, extensions=extensions, output_format="html5")
            ...
            html_content = bleach.clean(html_content, tags=allowed_tags, attributes=allowed_attrs, strip=False)
