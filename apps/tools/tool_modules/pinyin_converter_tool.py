...
class PinyinConverterForm(forms.Form):
    ...
    text = forms.CharField(
        label="输入中文",
        widget=forms.Textarea(attrs={"class": "form-control", "rows": 4, "placeholder": "请输入要转换的中文..."}),
        required=True,
    )
    tone = forms.BooleanField(
        label="带声调", initial=True, required=False, widget=forms.CheckboxInput(attrs={"class": "form-check-input"})
    )
