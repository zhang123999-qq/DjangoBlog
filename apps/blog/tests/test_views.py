...
class TestCommentCreateView:
    ...
    def test_guest_can_comment(self):
        ...
        response = self.client.post(
            self.comment_url, {"content": "游客评论内容", "name": "游客", "email": "guest@example.com"}
        )
        # 应该重定向到文章详情页
    ...
    def test_authenticated_user_can_comment(self):
        ...
        response = self.client.post(self.comment_url, {"content": "登录用户评论内容"})
        # 应该重定向到文章详情页
        assert response.status_code == 302
        # 检查评论是否创建
        assert Comment.objects.filter(post=self.post).exists()
    ...
    def test_comment_on_nonexistent_post_fails(self):
        ...
        response = self.client.post(url, {"content": "评论内容"})
        # 应该返回404
...
class TestRateLimiting:
    ...
    def test_comment_rate_limit(self):
        ...
        # 发送10条评论(应该成功)
        for i in range(10):
            response = self.client.post(self.comment_url, {"content": f"评论内容 {i}"})
            assert response.status_code == 302

        # 第11条评论应该被限流
        response = self.client.post(self.comment_url, {"content": "第11条评论"})
    ...
    def test_rate_limit_by_ip(self):
        ...
        self.client.login(username="testuser", password="testpass123")
        for i in range(10):
            response = self.client.post(self.comment_url, {"content": f"用户1评论 {i}"})
            assert response.status_code == 302

        # 第二个用户尝试评论(相同IP)
        self.client.login(username="testuser2", password="testpass123")
        response = self.client.post(self.comment_url, {"content": "用户2评论"})
        # 应该被限流(相同IP)
        assert response.status_code == 429
