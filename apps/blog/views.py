        ...
        categories = list(
            Category.objects.annotate(
                published_count=Count(
                    "posts",
                    filter=models.Q(posts__status="published", posts__slug__isnull=False) & ~models.Q(posts__slug=""),
                )
            ).only("name", "slug")
        )
...
class PostListView(ListView):
    ...
    def get_queryset(self):
        ...
        queryset = (
            ...
            .prefetch_related(Prefetch("tags", queryset=Tag.objects.only("name", "slug")))
            .only(
                "title",
                "slug",
                "summary",
...
class PostDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # 不再执行额外查询
        context["comments"] = [c for c in self.object.comments.all() if c.review_status == "approved"]
        # 评论表单
...
def like_comment_view(request, comment_id):
    ...
    if comment.review_status != "approved":
        return JsonResponse({"success": False, "message": "只能对已审核通过的评论点赞"})
    ...
    return JsonResponse({"success": True, "liked": liked, "like_count": comment.like_count, "message": "操作成功"})
...
def post_draft_list(request):
    ...
    return render(
        ...
        {"posts": page_obj, "categories": categories, "tags": tags, "title": "我的草稿"},
    )
...
def my_posts(request):
    ...
    return render(
        request, "blog/my_posts.html", {"posts": page_obj, "categories": categories, "tags": tags, "title": "我的文章"}
    )
