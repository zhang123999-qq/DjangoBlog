...
def staff_user():
    """管理员用户"""
    user = User.objects.create_user(username="staff", email="staff@example.com", password="StaffPassword123!")
    user.is_staff = True
    user.save()
    return user
...
def board():
    """测试版块"""
    return Board.objects.create(name="技术讨论", description="技术交流版块")
...
def topic(user, board):
    ...
    return Topic.objects.create(
        board=board, author=user, title="测试主题标题", content="这是测试主题的内容", review_status="approved"
    )
...
class TestPostAPI:
    ...
    def test_post_comments(self, api_client, post, user):
        """测试文章评论列表"""
        # 创建评论
        Comment.objects.create(post=post, user=user, content="测试评论", review_status="approved")
    ...
    def test_post_comments_do_not_expose_email_addresses(self, api_client, post, user):
        """公开评论接口不应暴露账号或游客邮箱"""
        Comment.objects.create(post=post, user=user, content="authenticated comment", review_status="approved")
        Comment.objects.create(
            post=post, name="guest", email="guest@example.com", content="guest comment", review_status="approved"
        )
    ...
    def test_draft_post_not_visible(self, api_client, user, category):
        """测试草稿不可见"""
        draft = Post.objects.create(title="草稿文章", content="内容", author=user, category=category, status="draft")
...
class TestTopicAPI:
    ...
    def test_topic_replies(self, api_client, topic, user):
        """测试主题回复列表"""
        Reply.objects.create(topic=topic, author=user, content="测试回复", review_status="approved")
    ...
    def test_pending_topic_not_visible(self, api_client, board, user):
        ...
        pending = Topic.objects.create(
            board=board, author=user, title="待审核主题", content="内容", review_status="pending"
        )
...
class TestSearchAPI:
    ...
    def test_pending_topic_not_visible_in_global_search(self, api_client, board, user):
        pending = Topic.objects.create(
            board=board, author=user, title="hidden global topic", content="hidden global body", review_status="pending"
        )
