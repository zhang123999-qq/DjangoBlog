...
class TopicDetailView(DetailView):
    ...
    def get_queryset(self):
        ...
        approved_replies = (
            Reply.objects.filter(is_deleted=False, review_status="approved")
            .select_related("author")
            .only("id", "content", "created_at", "like_count", "author__id", "author__username", "author__nickname")
        )
    ...
    def get_context_data(self, **kwargs):
        """获取上下文数据"""
        context = super().get_context_data(**kwargs)
        context["replies"] = list(self.object.replies.all())
...
def like_reply_view(request, reply_id):
    ...
    try:
        ...
        if reply.review_status != "approved":
            return JsonResponse({"success": False, "message": "只能对已审核通过的回复点赞"})
        ...
        return JsonResponse({"success": True, "liked": liked, "like_count": reply.like_count, "message": "操作成功"})
    except Exception as e:
        logger.error(f"点赞回复失败: {e}", exc_info=True)
        return JsonResponse({"success": False, "message": "操作失败，请稍后重试"}, status=500)
