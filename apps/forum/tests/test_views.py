...
class TestBoardListView:
    ...
    def setup_method(self):
        """设置测试数据"""
        self.client = Client()
        self.board = Board.objects.create(name="技术讨论", slug="tech", description="技术讨论版块")
...
class TestTopicListView:
    ...
    def setup_method(self):
        ...
        self.topic = Topic.objects.create(
            board=self.board, author=self.user, title="测试主题", content="测试内容", review_status="approved"
        )
    ...
    def test_topic_list_view_only_approved(self):
        ...
        Topic.objects.create(
            board=self.board, author=self.user, title="未审核主题", content="未审核内容", review_status="pending"
        )
...
class TestTopicDetailView:
    ...
    def test_topic_detail_view_only_approved_replies(self):
        """测试主题详情只显示已审核回复"""
        # 创建未审核回复
        Reply.objects.create(topic=self.topic, author=self.user, content="未审核回复", review_status="pending")

        # 创建已审核回复
        Reply.objects.create(topic=self.topic, author=self.user, content="已审核回复", review_status="approved")
...
class TestTopicCreateView:
    ...
    def test_topic_create_view_post(self):
        ...
        response = self.client.post(
            self.topic_create_url, {"title": "测试主题标题", "content": "测试主题内容，至少10个字符"}
        )
        # 应该重定向到版块列表
    ...
    def test_topic_create_view_invalid_form(self):
        ...
        self.client.login(username="testuser", password="testpass123")
        response = self.client.post(
            self.topic_create_url, {"title": "短", "content": "测试主题内容，至少10个字符"}  # 标题太短
        )
        # 应该返回表单页面（带错误）
        assert response.status_code == 200
        assert "form" in response.context
    ...
    def test_topic_create_view_nonexistent_board(self):
        ...
        response = self.client.post(url, {"title": "测试主题标题", "content": "测试主题内容，至少10个字符"})
        assert response.status_code == 404
...
class TestReplyCreateView:
    ...
    def test_reply_create_view_post(self):
        """测试回复POST请求"""
        self.client.login(username="testuser", password="testpass123")
        response = self.client.post(self.reply_create_url, {"content": "测试回复内容，至少5个字符"})
        # 应该重定向到主题详情
        assert response.status_code == 302
    ...
    def test_reply_create_view_invalid_form(self):
        """测试无效回复表单"""
        self.client.login(username="testuser", password="testpass123")
        response = self.client.post(self.reply_create_url, {"content": "短"})  # 回复太短
        # 应该重定向回主题详情（表单验证失败）
        assert response.status_code == 302
    ...
    def test_reply_create_view_nonexistent_topic(self):
        ...
        response = self.client.post(url, {"content": "测试回复内容，至少5个字符"})
        assert response.status_code == 404
...
class TestLikeReplyView:
    ...
    def setup_method(self):
        ...
        self.reply = Reply.objects.create(
            topic=self.topic, author=self.user, content="测试回复", review_status="approved"
        )
...
class TestPermissionControl:
    ...
    def setup_method(self):
        ...
        self.board = Board.objects.create(name="技术讨论", slug="tech", description="技术讨论版块")
        self.topic = Topic.objects.create(
            board=self.board, author=self.user1, title="测试主题", content="测试内容", review_status="approved"
        )
        self.reply = Reply.objects.create(
            topic=self.topic, author=self.user1, content="测试回复", review_status="approved"
        )
    ...
    def test_unapproved_reply_cannot_like(self):
        ...
        unapproved_reply = Reply.objects.create(
            topic=self.topic, author=self.user1, content="未审核回复", review_status="pending"
        )
    ...
    def test_only_approved_topics_visible(self):
        ...
        # 创建未审核主题
        Topic.objects.create(
            board=self.board, author=self.user1, title="未审核主题", content="未审核内容", review_status="pending"
        )
    ...
    def test_only_approved_replies_visible(self):
        """测试只有已审核回复可见"""
        # 创建未审核回复
        Reply.objects.create(topic=self.topic, author=self.user1, content="未审核回复", review_status="pending")
