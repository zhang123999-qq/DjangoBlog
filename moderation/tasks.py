...
def async_moderate_text(self, content_type: str, content_id: int, content: str, user_id: Optional[int] = None):
    """异步文本审核。"""
    ...
    try:
        ...
        if obj:
            ...
            ModerationLog.objects.create(
                ...
                action=(
                    ...
                    else "rejected" if result["status"] == "rejected" else "reminded"
                ),
                note=result["message"],
            )
        ...
        _log_task_error(
            "async_moderate_text.database", e, content_type=content_type, content_id=content_id, user_id=user_id
        )
...
def async_moderate_image(
    ...
    try:
        ...
        _log_task_error(
            "async_moderate_image.database", e, content_type=content_type, content_id=content_id, user_id=user_id
        )
        raise self.retry(exc=e)
    except Exception as e:
...
def check_pending_moderation():
    ...
    try:
        ...
        pending_comments = Comment.objects.filter(review_status="pending", created_at__lt=threshold).values_list(
            "id", flat=True
        )
